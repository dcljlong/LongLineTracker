...
function showCheckoutModal(equipmentId, action) {
    ...
    siteSelect.innerHTML = '<option value="">Select Site</option>' + 
        ...
        // Expected return date field (optional)
    const expectedEl = document.getElementById('checkout-expected-return');
    if (expectedEl) {
        expectedEl.value = '';
        if (action === 'checkout') {
            // if there is an active checkout, preload its expected return
            const checkouts = getCheckouts();
            const active = checkouts.find(c => c.equipmentId === equipmentId && !c.returnedAt);
            if (active && active.expectedReturnAt) expectedEl.value = active.expectedReturnAt;
        }
    }
...
function saveCheckout(event) {
    ...
    const expectedReturnAt = (form.expectedReturnAt && form.expectedReturnAt.value) ? form.expectedReturnAt.value : '';
const equipment = getEquipment();
    const checkouts = getCheckouts();
    ...
    if (action === 'checkout') {
        ...
        if (eqIndex !== -1) {
            ...
            equipment[eqIndex].expectedReturnDate = expectedReturnAt || '';if (site) equipment[eqIndex].site = site;
            saveEquipment(equipment);
        }
        ...
        if (eqIndex !== -1) {
            ...
            equipment[eqIndex].expectedReturnDate = '';if (site) equipment[eqIndex].site = site;
            saveEquipment(equipment);
        }
...
function loadOverdueReturns(equipment) {
    const el = document.getElementById('overdue-returns');
    if (!el) return;
    ...
    (checkouts || []).forEach(c => {
        ...
        if (!c.expectedReturnAt) return;

        const due = new Date(c.expectedReturnAt);
        if (isNaN(due.getTime())) return;

        if (due < now) {
            const eq = (equipment || []).find(e => e.id === c.equipmentId) || {};
            const days = Math.ceil((now.getTime() - due.getTime()) / msDay);

            overdue.push({
                name: eq.name || 'Unknown',
                assetId: eq.assetId || 'N/A',
                days,
    ...
    if (overdue.length === 0) {
        el.innerHTML = <div class="alert-item info"><i class="fas fa-check-circle"></i><div><strong>None overdue</strong><div class="alert-time">All returns up to date</div></div></div>;
        return;
    }

    overdue.sort((a, b) => b.days - a.days);

    el.innerHTML = overdue.slice(0, 5).map(item => {
        const label = ${item.name} ();
        const who = item.personName ?  •  : '';
        const text = ${item.days} day overdue;
        return <div class="alert-item danger"><i class="fas fa-exclamation-triangle"></i><div><strong></strong><div class="alert-time"></div></div></div>;
    }).join('');
}
